...
from lolbo.utils.mol_utils.selfies_vae.model_positional_unbounded import InfoTransformerVAE
from lolbo.vae import get_trained_selfies_vae
from scales.utils.data_utils.utils import get_dataset
from scales.nets.vae_template import VAE
...
class TransformerVAE(VAE):
    def __init__(self, trained_vae: InfoTransformerVAE):
        super(TransformerVAE, self).__init__(encoder=trained_vae.encoder,
    ...
    def decode(self, z, eval=True):
        ...
        z = z.reshape(-1, self.trained_vae.bottleneck_size, self.trained_vae.d_model)
        self.trained_vae.eval()
        self.trained_vae.decoder = self.trained_vae.decoder.to(device=self.device, dtype=torch.float32)
    ...
    def get_decoding_quality(self, z, return_smiles=False):
...
if __name__ == '__main__':
    ...
    for i in range(n_batches):
        ...
        print(z.shape)
        quality = vae.get_decoding_quality(z)
        ...
        print(f"Recon Low quality: {recon_loss_low}, Recon High quality: {recon_loss_high}")
    ...
    print(f"mean loss low quality: {total_loss_low / total_n_low}\n mean loss high quality: {total_loss_high / total_n_high}")
    print(f"mean recon loss low quality: {total_recon_loss_low / total_n_low}\n mean recon loss high quality: {total_recon_loss_high / total_n_high}")
