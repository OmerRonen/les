...
class SELFIESDataModule(LightningDataModule):
    def __init__(
        ...
        self.train = SELFIESDataset(train_data_path)
        self.val   = SELFIESDataset(validation_data_path)
...
def collate_fn(data):
    ...
    data_long =  torch.vstack(
        # Pad with stop token
        [F.pad(x, (0, max_size - x.shape[-1]), value=1) for x in data]
    )
    return data_long
    # data_one_hot = F.one_hot(data_long, num_classes=len(DEFAULT_SELFIES_VOCAB)).float()
    # return data_one_hot